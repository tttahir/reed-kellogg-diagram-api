...
class rule_1(rule):
    ...
    def __init__(self):
        ...
        self.action_set = {"nsubj": 0, "nsubjpass": 0,
                           "csubj": 0, "csubj": 0, "csubjpass": 0, "dobj": 1}
...
class rule_3(rule):
    ...
    def __init__(self):
        self.rule_num = 3
        self.set_rules = {"advmod", "neg", "nmod", "appos", "det", "infmod", "nn", "num",
                          "partmod", "possessive", "predet", "pcomp", "number", "amod", "meta", "poss"}
        self.action_set = {key: 0 for key in self.set_rules}
        self.rule_draw = [lambda child,
                          root: "__"+root+"__\n\t\\\n\t \\"+child]
...
class rule_5(rule):
    ...
    def __init__(self):
        ...
        self.rule_draw = [lambda child, root: "__" +
                          root+"__\n\t\\\n\t \\\n\t  \\__"+child+"__"]
...
class rule_7(rule):
    ...
    def __init__(self):
        self.rule_num = 7
        self.set_rules = {"relcl", "npadvmod",
                          "rcmod", "quantmod", "advcl", "mark"}
        self.action_set = {"relcl": 0, "npadvmod": 0,
                           "rcmod": 0, "quantmod": 0, "advcl": 0, "mark": 1}
        self.rule_draw = [lambda child, root: "__first_sentence_root:_ " + root + "__\n\t|\n\t|\n\t| \n  __second_sentence_root:_" +
                          child + "__", lambda child, root:"__first_sentence_root:_ " + root + "__\n\t|\n\t"+child+"\n\t|"]
...
class rule_8(rule):
    ...
    def __init__(self):
        ...
        self.action_set = {"relcl": 0, "npadvmod": 0,
                           "rcmod": 0, "quantmod": 0, "advcl": 0, "mark": 0}
        self.rule_draw = [lambda child, root: "__first_sentence_root:__" + root +
...
class relations_map():
    def __init__(self):
        self.rules = [rule_1(), rule_2(), rule_3(), rule_4(), rule_5(), rule_6(
        ), rule_7(), rule_8(), rule_9(), rule_10(), rule_merge(), rule_default()]
...
def modify_graph(root, nodes_relation, graph_root, all_childs=None):
    ...
    diagram_relation_type = decide_rule(
        relation_type, root, child, all_childs=all_childs)
    child_graph_node = GraphNode(text=diagram_relation_type, parent=graph_root,
                                 value=child.orth_, raw_relation=relation_type, meta=child)
...
def construct_sent_diagram(sentence):
    ...
    while len(visited_nodes) > 0:
        ...
        for relation in nodes_relation:
            child_graph_node = modify_graph(
                root, relation, root_graph_node, all_childs=nodes_relation)
            ...
            if isinstance(child_graph_node.text, rule_10):
                ...
                # set(root.subtree)
                root_subtree = subtrees[str(
                    root.orth_)+"_"+str(root.i)]["nodes"]

                child_root_subtree = set(relation[0].subtree)
                subtrees[str(root.orth_) + "_" + str(root.i)] = {"nodes": root_subtree - child_root_subtree, "parent": subtrees[str(root.orth_) + "_" + str(
                    root.i)]["parent"], "childs": subtrees[str(root.orth_) + "_" + str(root.i)]["childs"]+[str(relation[0].orth_) + "_" + str(relation[0].i)], "r_node": root_graph_node}
                subtrees[str(relation[0].orth_) + "_" + str(relation[0].i)] = {"nodes": child_root_subtree, "parent": str(
                    root.orth_) + "_" + str(root.i), "childs": [], "r_node": child_graph_node}
...
def draw_tree(node, id):
    ...
    print('=====================================================')
    if node.parent is None:
        print('nesting:', id, '|||| value:',
              node.value, '|||| pvalue:', 'root')
        ...
        tag = node.meta.tag_
        print('rule(' + str(r) + ',' + str(sr) + ')', '|||| nesting:',
              id, '|||| value:', v, '|||| pvalue:', pv)
            ...
            if pr == r == 3:
                ...
                if i + 1 < len(ch):
                    if r == ch[i + 1]['rule'] == 3:
                        ...
                        ch[i + 1]['subRule'] = 1
                        ch[i]['value'], ch[i + 1]['value'] = [ch[i + 1]
                                                              ['value'], ch[i]['value']]
